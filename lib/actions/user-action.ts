        ...
        if (!user) {
            user = await prisma.user.create({
                data: {
                    ...
                    name: clerkUser.fullName || clerkUser.firstName || clerkUser.lastName || "New User",
                    image: clerkUser.imageUrl || null,
                    tokens: getPlanTokens("PRO"),
                    plan: "PRO",
                },
            })
        }
...
export async function getUserUsage() {
    try {
        ...
        const planLimits = {
            STARTER: { assistants: 5, interactions: 1000, teamMembers: 1, monthlyTokens: 10000 },
            PRO: { assistants: 25, interactions: 10000, teamMembers: 10, monthlyTokens: 50000 },
            ENTERPRISE: { assistants: -1, interactions: -1, teamMembers: -1, monthlyTokens: -1 }, // unlimited
        }
...
function getPlanTokens(plan: string): number {
    ...
    return planTokens[plan as keyof typeof planTokens] || 50000
}
